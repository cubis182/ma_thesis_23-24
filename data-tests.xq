...
('CASE,WORK,SENT-ADDR,TEXT,TOK-ADDR,LEM,TYPEA,TYPEB,SENTLEN,WORKLEN'),
...
return fn:string-join(($n, $work, deh:get-sent-address($tree), $text, deh:get-tok-address($item?1), $item?1/fn:replace(deh:process-lemma(fn:string(@lemma)), ",", ""), $item?2, $item?3, $work-length, $total-length), ",")