...
(:Temporal clauses are different because, for the second item in each array, we want the string for the separable temporal conjunctions if there is one, not the normal lemma:)
let $temporal := deh:temporal-clause($clause-pairs) ! array{.?1/deh:get-tok-address(.), ((if (array:size(.) > 2) then (deh:process-lemma(.?3)) else (deh:process-lemma(.?1/fn:string(@lemma)))) || '_temp'), .?2[1]/deh:process-lemma(fn:string(@lemma)), "temp"}
